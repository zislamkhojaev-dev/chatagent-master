...
async def put_scenarios(body: dict, _: None = Depends(verify_admin)):
    try:
        config = save_scenarios(body)
        return {"status": "success", "updated_at": config.updated_at, "count": len(config.scenarios)}
...
async def post_scenarios_reload(_: None = Depends(verify_admin)):
