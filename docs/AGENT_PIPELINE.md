...
| `if_ambiguous` | Вопрос если KB пустая/неоднозначная **или** незаполнен слот `user_type` / `payment_channel` (даже при sufficient на default audience) |
...
**Сначала** поиск в KB, **потом** уточнение. Если слот `user_type` / `payment_channel` ещё пуст, бот всё равно спрашивает — иначе после разметки БЗ под `client` уточнения пропадали бы при любом «sufficient» hit.
